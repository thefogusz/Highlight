...
def test_timestamp_is_resolved_and_changes_job_identity(service, suffix):
    ...
    full = service.call('highlight_create', {'background_research': OFFLINE_RESEARCH, **{'url': url}})
    timed = service.call('highlight_create', {'background_research': OFFLINE_RESEARCH, **{'url': url + suffix}})
...
def test_explicit_start_overrides_link_and_bad_timestamp_rejected(service):
    ...
    result = service.call('highlight_create', {'background_research': OFFLINE_RESEARCH, **{'url': url, 'start_seconds': 0}})
    assert result['ok'] and result['resolved_options']['start_seconds'] == 0
    assert not service.call('highlight_create', {'background_research': OFFLINE_RESEARCH, **{'url': url.replace('574s', '-1')}})['ok']
...
def test_duplicate_submission_reuses_job(service):
    a = service.call("highlight_create", {'background_research': OFFLINE_RESEARCH, **{"url": "https://youtu.be/abcdefghijk"}})
    b = service.call("highlight_create", {'background_research': OFFLINE_RESEARCH, **{"url": "https://www.youtube.com/watch?v=abcdefghijk"}})
    assert a["job_id"] == b["job_id"] and b["reused"]


def test_idempotency_conflict(service):
    service.call("highlight_create", {'background_research': OFFLINE_RESEARCH, **{"url": "https://youtu.be/abcdefghijk", "idempotency_key": "one"}})
    result = service.call("highlight_create", {'background_research': OFFLINE_RESEARCH, **{"url": "https://youtu.be/12345678901", "idempotency_key": "one"}})
    assert result["error"]["code"] == "IDEMPOTENCY_CONFLICT"


def test_invalid_duration_does_not_queue(service):
    result = service.call("highlight_create", {'background_research': OFFLINE_RESEARCH, **{"url": "https://youtu.be/abcdefghijk", "min_duration_seconds": 100, "max_duration_seconds": 20}})
...
def test_highlights_default_to_one_minute_maximum(service):
    ...
    result = service.call('highlight_create', {'background_research': OFFLINE_RESEARCH, **{'url': 'https://youtu.be/abcdefghijk', 'max_duration_seconds': 61}})
...
def test_five_minute_ceiling_does_not_change_next_job_default(service):
    custom=service.call('highlight_create',{'background_research': OFFLINE_RESEARCH, **{'url':'https://youtu.be/abcdefghijk','max_duration_seconds':300}})
    normal=service.call('highlight_create',{'background_research': OFFLINE_RESEARCH, **{'url':'https://youtu.be/abcdefghijk'}})
...
def test_reused_failed_job_is_identified_as_historical(service):
    ...
    created = service.call('highlight_create', {'background_research': OFFLINE_RESEARCH, **request})
    service.store.update(created['job_id'], state='failed', error='old two hour limit')
    result = service.call('highlight_create', {'background_research': OFFLINE_RESEARCH, **request})
