...
async def serve():
    ...
    server = Server("highlight", icons=icons, website_url="https://github.com/thefogusz/Highlight", instructions="For a YouTube highlight request, check highlight_settings, then call highlight_create once with the user's URL and options. Poll highlight_status at the suggested interval and return actual MP4 paths from highlight_results. Show its dashboard_path in the host HTML preview if available; otherwise summarize usage in Thai. The local call cap is not Google quota; unknown values must stay unknown. Ask for a URL only if missing. Never ask for API keys in chat; use local Highlight Settings. Do not automatically retry provider failures. Heatmap is optional replay intensity, not viewer count or proof of humor.")
