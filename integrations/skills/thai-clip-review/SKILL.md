...
When ingest fails, the host agent owns recovery. Use an available supported acquisition tool in a genuinely different working environment; do not repeat the same blocked command. Preserve the job and its research/options. Do not default to asking the user for MP4. Once a full source is obtained, verify its exact YouTube identity, original timeline, duration, and best available quality (prefer 1080p+, minimum 720p). Call `highlight_retry` with `acquired_source: {path, source_url, expected_duration_seconds, method}`. Use an absolute local path and a short non-secret method description. The server probes, remuxes and decodes video/audio, then resumes the same job. URL and duration are host attestations, not content fingerprint verification. Never invent identity or use a substitute clip. Source import is not proof an alternate YouTube downloader succeeded. Keep subtitle-first transcription and full-story review. Respect access restrictions and tool-policy blocks; request user action only when genuinely necessary.
