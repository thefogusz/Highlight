...
โมเดลหลักคือ **Gemini 3.6 Flash** (`gemini-3.6-flash`) หน้าตั้งค่าจะเลือกเป็นอันดับแรกเมื่อบัญชีมองเห็นรุ่นนี้ และยังแสดงรุ่นอื่นที่รองรับให้เลือก ค่าที่ผู้ใช้เลือกไว้ชัดเจนจะคงเดิม ไม่อัปเกรดเพียงเพราะมีรุ่นใหม่กว่า รายชื่อโมเดลไม่ได้รับประกันว่า generate จะสำเร็จ และระบบยังไม่สลับโมเดลระหว่างงานโดยอัตโนมัติ
...
### เวลาเริ่มและสถานะการวิเคราะห์

ลิงก์ที่มี `t=574s`, `t=9m34s` หรือ `start=574` จะคัดไฮไลต์ตั้งแต่ 9:34 จนจบวิดีโอ โดยเวลาในคลิปยังอ้างอิงวิดีโอต้นฉบับ Agent ดูค่าที่ใช้จริงได้จาก `resolved_options.start_seconds` หากต้องการทั้งคลิป ให้ส่ง `start_seconds: 0` ชัดเจน ช่วง `focus_ranges` จะถูกจำกัดด้วยเวลาเริ่มนี้ด้วย การดาวน์โหลดและเตรียมบทสนทนายังคงใช้ทั้งวิดีโอ

สถานะ `discover` หมายถึงคัดช่วงจากบทสนทนาและกราฟดูซ้ำที่ดึงได้ ส่วน `inspect` จึงเริ่มตรวจภาพและเสียงของช่วงที่เลือก ห้ามรายงานว่าตรวจภาพ/เสียงแล้วเพียงเพราะเข้าขั้น discover

ข้อผิดพลาดผู้ให้บริการแยก HTTP 400 (คำขอ), 401/403 (คีย์หรือสิทธิ์), 404 (โมเดลหรือทรัพยากร), 429 (อัตราเรียกหรือโควตา), 5xx (บริการ), เครือข่าย/timeout และ JSON ใช้ไม่ได้ โดยไม่บันทึกข้อความตอบกลับดิบหรือคีย์ เฉพาะ HTTP 503 จากการวิเคราะห์โมเดลจะลองซ้ำได้อีก 2 ครั้ง โดยรอประมาณ 10 และ 20 วินาทีพร้อมสุ่มเวลาหน่วงเล็กน้อย ทุกครั้งนับรวมในเพดาน 30 calls และยกเลิกระหว่างรอได้ ข้อผิดพลาดอื่นไม่ลองซ้ำอัตโนมัติ และ 429 ไม่ได้ยืนยันว่าเครดิตฟรีหมด ข้อผิดพลาดงานเก่าจะยังคงเป็นข้อมูลเดิม ไม่สามารถระบุสาเหตุย้อนหลังได้

หลังอัปเดตแพ็กเกจ ให้เชื่อมต่อ Highlight MCP ใหม่หรือปิดเปิดแอป agent เพื่อให้ process เดิมโหลดโค้ดและ tool schema ใหม่
...
ช่วงเวลาที่ระบุจะตรวจขอบเขตกับความยาวไฟล์จริงจาก ffprobe แทนความยาว metadata ที่อาจปัดเศษ หากปลายช่วงเกินไฟล์จริงไม่เกิน 0.25 วินาทีจะตัดให้พอดี ส่วนที่เกินกว่านั้นยังปฏิเสธตามเดิม คลิปไฮไลต์ยังจำกัดไม่เกิน 60 วินาที
