...
ทุกไฮไลต์เป็นไฟล์แยก ความยาวไม่เกิน 60 วินาที รวมการสั่งแก้คลิป ไม่รวมหลายช็อตเป็นคลิปยาว
...
เมื่อเริ่มงานหรือ retry หาก metadata ที่เก็บไว้ไม่มี heatmap และยังไม่ได้คัดช่วง ระบบลองดึง metadata สดอีกหนึ่งครั้งโดยไม่ใช้ไฟล์แคชของ Highlight ไม่ดาวน์โหลดวิดีโอซ้ำและไม่เรียก Gemini เพื่อเช็กกราฟ หากดึงไม่สำเร็จจะแจ้งแยกจากกรณีดึงข้อมูลได้แต่ไม่มีกราฟส่งมา โหมด `ignore` ไม่ลองเพิ่ม และโหมด `require` ยังหยุดหากไม่มีกราฟ งานที่มี candidates เก็บไว้แล้วใช้ผลเดิมต่อเพื่อไม่ให้วิเคราะห์แบบเสียเงินซ้ำโดยอัตโนมัติ
...
### ความคืบหน้าการถอดเสียง

ก่อนเริ่ม Whisper ระบบลองดึงซับไทยเจ้าของช่อง แล้วซับไทยอัตโนมัติ โดยไม่ใช้ซับแปลอัตโนมัติ ตรวจเวลา ข้อความไทย และช่วงเวลาครอบคลุมต้นคลิปจนอย่างน้อย 90% ของความยาว หากไม่มี ดึงไม่ได้ หรือซับขาดช่วงท้ายมาก จะใช้ Whisper พร้อม checkpoint แทน ซับที่ผ่านเกณฑ์เวลาไม่ได้หมายความว่าข้อความถูกต้องทุกคำ งานที่มี transcript สมบูรณ์เก็บไว้ใช้ต่อโดยไม่ดึงซ้ำ

ถอดเสียงเป็นช่วงละ 2 นาทีพร้อมบริบทรอบขอบช่วง 2 วินาที บันทึก checkpoint หลังแต่ละช่วง และรายงานเวลาที่บันทึกแล้วใน dashboard กับ `highlight_status.transcription` หากหยุดจะทำซ้ำเฉพาะช่วงที่ยังไม่บันทึก ไม่ใช่ทั้งตอน งานเก่าที่มี transcript สมบูรณ์ใช้ต่อได้ ช่วงแรกอาจรอโหลดโมเดลก่อนมีความคืบหน้า ตัวเลขนี้เป็นความคืบหน้าการถอดเสียง ไม่ใช่เปอร์เซ็นต์งานทั้งหมด Agent ควรรอจริงอย่างน้อย 60 วินาทีระหว่างตรวจสถานะ ไม่ยิงคำขอเป็นชุด
